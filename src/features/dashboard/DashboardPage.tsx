            ...
            for(let mutation of mutationsList) {
                if (mutation.type === 'attributes' && mutation.attributeName === 'class') {
                    if (chartRef.current) {
                        ...
                        chartRef.current.options.plugins.legend.labels.color = newTextColor;
                        chartRef.current.options.plugins.title.color = newTextColor;
                        chartRef.current.options.scales.y.ticks.color = newTextColor;
                        chartRef.current.options.scales.x.ticks.color = newTextColor;
                        chartRef.current.options.scales.y.grid.color = newGridColor;
                        chartRef.current.options.scales.x.grid.color = newGridColor;
                                        ...
                                        <li key={m.id} className="bg-gray-50 dark:bg-gray-700 p-4 rounded-lg shadow-sm border-l-4 border-blue-500">
                                            <div className="flex justify-between items-start">
                                                <div>
                                                    ...
                                                    <p className="text-sm text-gray-600 dark:text-gray-300"><span className="font-semibold">Horário:</span> {new Date(m.dateTime).toLocaleString('pt-BR', { hour: '2-digit', minute: '2-digit' })}</p>
